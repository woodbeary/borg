export const SAFETY_TIPS = [
  {
    title: "Minimum Squad Size",
    tip: "Never BORG alone - maintain a minimum of 3 BORGanists for optimal safety",
    icon: "👥"
  },
  {
    title: "Hydration Balance",
    tip: "For every BORG hour, consume one regular water. Stay hydrated, stay safe",
    icon: "💧"
  },
  {
    title: "Sacred Ratios",
    tip: "The holy ratio: 60% water, 30% chosen enhancement, 10% electrolytes",
    icon: "⚗️"
  }
];

export const BORG_TERMS: Definition[] = [
  {
    term: "Borgasm",
    definition: "When you tap out from drinking too much, but only when you really really cant have any more",
    author: "Original BORGanist",
    createdAt: "2024-01-01T00:00:00Z",
    votes: 100,
    isStatic: true
  },
  {
    term: "Borganism",
    definition: "4 or more people in a circle, drinking a BORG. The sacred ritual of hydration",
    author: "Original BORGanist",
    createdAt: "2024-01-01T00:00:00Z",
    votes: 90,
    isStatic: true
  },
  {
    term: "Borgtender",
    definition: "The designated BORG creator for the group. Handles the sacred water-to-enhancement ratios",
    author: "Original BORGanist",
    createdAt: "2024-01-01T00:00:00Z",
    votes: 80,
    isStatic: true
  },
  {
    term: "Borgpocalypse",
    definition: "When everyone in the Borganism hits their Borgasm simultaneously",
    author: "Original BORGanist",
    createdAt: "2024-01-01T00:00:00Z",
    votes: 70,
    isStatic: true
  }
]; 